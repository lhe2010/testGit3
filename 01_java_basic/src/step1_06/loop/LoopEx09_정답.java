package step1_06.loop;

import java.util.Scanner;

/*
 * # ATM[2단계]
 * 1. 입금
 * . 입금할 금액을 입력받아, myMoney에 입금
 * 2. 출금
 * . 출금할 금액을 입력받아, myMoney에서 출금
 * . 단, 출금할 금액이 myMoney 잔액을 초과할 경우 출금불가
 * 3. 이체
 * . yourAcc 계좌번호를 입력받아, 이체
 * . 이체할 금액이 myMoney 잔액을 초과할 경우 이체 불가
 * . 이체 후 yourMoney 잔액 증가
 * 4. 조회
 * . myMoney와 yourMoney 잔액 모두 출력
 */

public class LoopEx09_정답 {

	public static void main(String[] args) {
		
		Scanner scan = new Scanner(System.in);
		
		int myAcc = 1111;
		int myMoney = 50000;
		
		int yourAcc = 2222;
		int yourMoney = 70000;	
		
		int transMoney, transAcc;
		
		boolean isRun = true;
		while(isRun) {
			
			System.out.println("1.입금");
			System.out.println("2.출금");
			System.out.println("3.이체");
			System.out.println("4.조회");
			System.out.println("0.종료");
			
			System.out.print("메뉴 선택 : ");
			int sel = scan.nextInt();
			
			if(sel == 1) {
				System.out.print("입금할 금액: ");
				transMoney = scan.nextInt();
				myMoney += transMoney;
			}
			else if(sel == 2) {
				System.out.print("출금할 금액: ");
				transMoney = scan.nextInt();
				if(transMoney <= myMoney) 
					myMoney -= transMoney;
				else 
					System.out.println("[오류] 출금 불가 - 잔액 부족");
				
			}
			else if(sel == 3) {
				System.out.print("이체할 계좌번호: ");
				transAcc = scan.nextInt();
				
				if(transAcc != 2222) {
					System.out.println("[오류] 이체 불가 - 존재하지않는 이체 계좌");
				} else {		// 2222의 계좌에 입금하는경우 
					System.out.print("출금할 금액: ");
					transMoney = scan.nextInt();
					
					if(transMoney > myMoney)
						System.out.println("[오류] 이체 불가 - 잔액 부족");
					else { // 잔액이 있는경우
						myMoney -= transMoney;
						yourMoney += transMoney;	
						System.out.println("[ 이체완료 ]");
					}
				}
			}
			else if(sel == 4) {
				System.out.println("[조회] 나의 잔액: " + myMoney);
				System.out.println("[조회] 너의 잔액: " + yourMoney);
			}
			else if(sel == 0) {
				isRun = false;
				System.out.println("[ 프로그램 종료 ]");
			}
		}


	}

}
